...
function normalizeR2Config(r2) {
  ensure(r2 && typeof r2 === 'object', 'Missing required field: r2');
  const bucket_name = String(r2.bucket_name || '').trim();
  const file_name = String(r2.file_name || '').trim();
  ensure(bucket_name.length > 0, 'Missing required field: r2.bucket_name');
  ensure(file_name.length > 0, 'Missing required field: r2.file_name');
  const raw_prefix = String(r2.prefix || '').trim();
...
async function writeVault(workerUrl, auth, firebaseApiKey, r2, blobBytes) {
  await refreshIdTokenIfNeeded(auth, firebaseApiKey);
  return jsonFetch(`${workerUrl}/vault/write`, {
    method: 'POST',
    ...
    body: JSON.stringify({
      ...
      payload_b64: bytesToB64(blobBytes),
    }),
  });
}
...
async function main() {
  ...
  // ── Phase 1: generate ──────────────────────────────────────────────────────
  console.log(`[PHASE 1] Generating ${ENTRY_COUNT} entries`);
  ...
  for (let i = 0; i < ENTRY_COUNT; i++) {
    ...
    // Vault entry format: flat HEAD fields + _commits (newest first).
    const commits = entry.history.commits;
    const headSnapshot = commits[0].snapshot;
    entries.push({
      id: entry.id,
      ...headSnapshot,
      _commits: commits,
  ...
  console.log('[PHASE 2] Compressing and encrypting vault blob');
  ...
  const compressed = brotliCompress(jsonBytes);
  const blobBytes = await encryptBytesToBlob(rootMasterKeyBytes, compressed);

  const encMs = performance.now() - encStart;
  console.log(
    `[PHASE 2] Done in ${formatDurationMs(encMs)} ` +
    `(json=${jsonBytes.length}B compressed=${compressed.length}B blob=${blobBytes.length}B)`
  );

  // ── Phase 3: upload ────────────────────────────────────────────────────────
  console.log('[PHASE 3] Uploading vault blob via POST /vault/write');
  const uploadStart = performance.now();
  const result = await writeVault(config.worker_url, auth, config.firebase_api_key, r2, blobBytes);
  const uploadMs = performance.now() - uploadStart;
  ensure(result && result.ok === true, `Vault write failed: ${JSON.stringify(result)}`);
  console.log(`[PHASE 3] Done in ${formatDurationMs(uploadMs)}`);

  const totalMs = genMs + encMs + uploadMs;
  ...
  console.log(`- Vault JSON size:                ${jsonBytes.length} bytes`);
  console.log(`- Compressed size:                ${compressed.length} bytes`);
  console.log(`- Encrypted blob size:            ${blobBytes.length} bytes`);
  console.log(`- Phase 1 (generate):             ${formatDurationMs(genMs)}`);
  console.log(`- Phase 2 (compress + encrypt):   ${formatDurationMs(encMs)}`);
  console.log(`- Phase 3 (upload):               ${formatDurationMs(uploadMs)}`);
