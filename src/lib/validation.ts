...
export function validateRootMasterKey(value: string): boolean {
  try {
...
function validateS3DestinationConfig(c: UnknownRecord, label: string): string[] {
  const errors: string[] = [];
...
function validateRequiredFields(c: UnknownRecord): string[] {
  const errors: string[] = [];
  ...
  if (!validateRootMasterKey(String(c.root_master_key || ''))) {
    errors.push('Root master key must be base64 and at least 256 bytes decoded');
  }
  return errors;
}

function validateR2Config(c: UnknownRecord): string[] {
  if (c.r2_config === undefined) return [];
  const errors: string[] = [];
  const r2 = c.r2_config && typeof c.r2_config === 'object' ? (c.r2_config as UnknownRecord) : {};
...
function validateS3Configs(c: UnknownRecord): string[] {
  if (c.s3_config === undefined) return [];
  if (!Array.isArray(c.s3_config)) return ['S3 destinations must be a list'];
  return c.s3_config.flatMap((entry: unknown, i: number) =>
    validateS3DestinationConfig(
...
// otherwise unused.
function validateBackupMasterKeyRequirement(c: UnknownRecord): string[] {
  const wantsCloudBackup = c.r2_config !== undefined || (Array.isArray(c.s3_config) && c.s3_config.length > 0);
  if (!wantsCloudBackup || validateRootMasterKey(String(c.backup_master_key || ''))) return [];
  return ['Backup master key must be base64 and at least 256 bytes decoded'];
}

export function validateConfig(config: unknown): string[] {
  const c: UnknownRecord = config && typeof config === 'object' ? (config as UnknownRecord) : {};
  return [
