  ...
  return (
    <div className="mb-3">
      <div className="d-flex align-items-center justify-content-between mb-1">
        <SectionLabel icon="bi-sticky" className="form-label text-muted small fw-semibold mb-0">
          Notes
        </SectionLabel>
        {!isEditing && (
          <button
      ...
      {isEditing ? (
        <>
          ...
          {value.length >= NOTES_MAX && (
            <div className="text-danger small mt-1">Notes cannot exceed {NOTES_MAX.toLocaleString()} characters</div>
          )}
        </>
      ) : (
        <div className="form-control bg-white" style={{ minHeight: 80, whiteSpace: 'pre-wrap' }}>
          {value ? (
            visible ? (
              ...
              <span className="text-muted">Hidden. Click eye to reveal.</span>
            )
          ) : (
