...
- `entries.entryFile` <-> `$files.entry` (one entry data file per entry); `onDelete: cascade`, deleting an entry deletes its data file automatically. Every save uploads the new file at a fresh path, then atomically deletes the previous file and links the new one in a single `db.transact` call (see docs/crypto.md, Entry Data File), so there is never an externally observable moment with zero or two files linked to the entry — intended cardinality is a genuine `has: 'one'`/`has: 'one'` link.
- `entries.historyFile` <-> `$files.entry` (an entry's current history file); same cascade and the same atomic delete-and-link swap pattern as `entryFile` above (see docs/crypto.md, History File) — also intended as `has: 'one'`/`has: 'one'`.

Both of the above share the cardinality that produced a persistent "already exists" rejection for `keyStore.owner` when linked to `$users` (see Uniqueness below); that bug was specific to linking into `$users` and predates the atomic delete-and-link pattern used here, so it isn't confirmed to recur for either `entries`<->`$files` link. Still needs confirming empirically once implemented — fall back to `has: 'many'` on the `entries` reverse side, cardinality enforced client side same as `keyStore`, if the atomic swap turns out not to satisfy the constraint live.
