...
- **Entry data** (`entries.entryFile`): one file per entry, wrapped via HKDF+AEAD under the entry's `entryKey`, containing everything about the entry — type, title, fields, tags, notes, `createdAt`, `updatedAt`, `deletedAt` (trash marker). `path` is `${auth.id}/entries/${entryId}.json`, stable for the entry's lifetime: a save re-uploads to that same path, which InstantDB Storage overwrites, rather than creating a new file and relinking, since entry data is meant to change in place the same way the `encryptedData` field it replaces did (see Permission rules below for why this doesn't need `$files.update`).
- **Entry history** (`entries.historyFile`): one file per entry holding that entry's entire history, wrapped via HKDF+AEAD under the same `entryKey`, whose plaintext is a JSON array of every kept commit (`commitHash`, timestamp, full entry snapshot at that commit). There is no `entryHistory` table; a commit is never its own row. `path` is `${auth.id}/entryHistory/${entryId}/${latestCommitHash}.json` — unlike the entry data file, every save produces a new path (the latest commit hash always changes) instead of overwriting, since history must stay create-or-delete-only, never edited in place (see Permission rules below for why).
...
- `entries.entryFile` <-> `$files.entry` (one entry data file per entry), `has: 'one'` on both sides, `onDelete: cascade`, deleting an entry deletes its data file automatically. Unlike `historyFile` below, this file is overwritten in place at a stable path (see Entities above), never replaced by a new upload-then-relink-then-delete-old sequence, so there's no equivalent brief window where two files are linked to the same entry — a genuine `has: 'one'`/`has: 'one'` link is the intended cardinality here. This is the same cardinality that produced a persistent "already exists" rejection for `keyStore.owner` when linked to `$users` (see Uniqueness below); that bug was specific to linking into `$users`, not confirmed to recur for an `entries`<->`$files` link, but worth confirming empirically once implemented before relying on it rather than falling back to the client-side-enforced cardinality pattern used elsewhere in this file.
...
InstantDB treats any action left unspecified as allow, not deny, so every action on every entity above must be listed explicitly. `$files.update` is `false` for both kinds of file: history files are created fresh (new path, new commit hash) or deleted, never edited in place (see docs/crypto.md, History File); entry data files are re-uploaded to their existing path instead, which per InstantDB's own Storage docs ("if the path already exists ... it will be overwritten") is the same `create`-class Storage upload operation as the first upload, not the separate `update` permission action that governs editing an existing `$files` row's `path`/custom columns directly via `db.transact` — something this app never does for either file kind. `$files.update: false` is therefore safe to leave blanket-false without blocking entry saves; this needs confirming against live behavior once implemented, since it isn't spelled out explicitly in InstantDB's docs. Storage permissions scope on `data.path` rather than `data.ref`, so the `auth.id` prefix in the path (see Entities above) is what stands in for the `entries`/`keyStore` rules' `data.ref('owner.id')` check.
