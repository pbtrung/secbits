...
`ensureKeyStore` still queries `$users` directly, by its own id, to find the caller's `keyStore` row. `fetchUserEntries` and `rotateUserMasterKey` no longer go through `$users` at all; entries scope through `keyStore` now (see Links below).

`keyStore`

- `keyBlob` (string): base64 AEAD blob, general purpose — the only value stored today is the UMK (User Master Key), wrapped via HKDF+AEAD under `root_master_key`.
- `keyType` (string): discriminates what `keyBlob` holds; `'umk'` is the only value used today. The table is deliberately general purpose, not UMK-specific, so a future key kind can reuse it instead of adding a new entity.

One `keyType: 'umk'` row per user (see Uniqueness below for how that's enforced, now scoped by `keyType` too, not just by owner). Cloud backups are encrypted under `backup_master_key`, a config only secret that never touches InstantDB in any form (see docs/crypto.md, Key Hierarchy); there is no `keyStore` row for it.
...
- `keyStore.owner` <-> `$users.keyStore` (one `keyType: 'umk'` key store row per user, in practice — `has: 'many'` on the `$users` reverse side, not `has: 'one'`; see Uniqueness below for why); `onDelete: cascade` on the `keyStore` side, deleting a user's account deletes their `keyStore` row(s).
- `entries.keyBlob` <-> `keyStore.entries` (many entries to one `keyStore` row — an entry's owner is whoever owns that `keyStore` row, transitively via `keyStore.owner` above), `has: 'one'` on the `entries` side / `has: 'many'` on the `keyStore` side, `required: true`, `onDelete: cascade` on the `entries` side, deleting a `keyStore` row deletes its linked entries. Replaces the old direct `entries.owner` <-> `$users.entries` link entirely: entries no longer link to `$users` at all, only to their own `keyStore` row. One fewer link targeting `$users`, which is exactly where the `connects to non existing entity` push bug below always hit.
...
This is the same cardinality that produced a persistent "already exists" rejection for `keyStore.owner` when linked to `$users` (see Uniqueness below); that bug was specific to linking into `$users`, so it isn't confirmed to recur here, but worth confirming empirically once implemented — fall back to `has: 'many'` on the `entries` reverse side, cardinality enforced client side same as `keyStore`, if it does. No `required: true` on the forward link either, unlike `entriesKeyStore` above: the `$files` row is created by a separate Storage upload call before the `db.transact` that links it, so for a moment after upload the file exists unlinked, which a `required` constraint could reject.

Three things about the new two-hop `entries -> keyStore -> $users` scoping are assumed, not confirmed, pending a live push and test: whether `data.ref('keyBlob.owner.id')` (a two-hop ref) resolves correctly in a `create` rule when the first hop (`keyBlob`) is being set in that very same transact rather than already existing beforehand; whether deleting a `$users` row now cascades two levels deep (`$users` → `keyStore` → `entries`) or InstantDB only cascades one hop per delete; and whether the nested `$: { where: { keyType: 'umk' } }` filter on a sub-relation (used throughout `src/db.ts`'s `keyStore` queries) behaves as expected. None of these have failed, they simply haven't been exercised against a live app yet.
...
entries:
  view/create/delete → auth.id in data.ref('keyBlob.owner.id')
  update              → auth.id in data.ref('keyBlob.owner.id') && !('keyBlob' in request.modifiedFields)
...
"One `keyStore` row per user" — more precisely, since `keyStore` became a general purpose keyed-blob table (see Entities above), "one `keyType: 'umk'` row per user" — is enforced client side only, not at the schema level. InstantDB has no composite or partial uniqueness constraint (only `.unique()` across an entire attribute, which would be wrong here: many different users can each have their own `keyType: 'umk'` row). A genuine `has: 'one'`/`has: 'one'` link was tried, matching the design intent, but it produced a persistent "already exists" uniqueness rejection on `owner` even after confirming zero existing `keyStore` rows, no leftover entities from diagnostic renames, and correct auth settling to the right user. Deleting and recreating the link fresh (rather than editing cardinality in place) did not resolve it either. Backend enforcement was backed off to `has: 'many'` on the `$users` side; `ensureKeyStore` in `src/db.ts` still queries its own row and treats finding more than one as a fatal error rather than silently picking one, which gives the same practical guarantee without the stuck constraint. Every query for the caller's own `keyStore` row (`queryOwnKeyStoreRows`) filters by `keyType: 'umk'` as well as by owner, so this fatal-error check is really "more than one `keyType: 'umk'` row", not "more than one row of any kind" — the table can grow a second `keyType` later without an unrelated row ever being mistaken for a duplicate UMK.
