...
- Database and session: InstantDB — `@instantdb/react` on the client; no admin SDK anywhere, since there is no server side component. An entry's data and its entire history live together in one InstantDB Storage file (`$files`), not a database field (see docs/data_model.md, Entities); `keyStore` and `entries` remain ordinary rows.
