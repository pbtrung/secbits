...
- Database and session: InstantDB — `@instantdb/react` on the client; no admin SDK anywhere, since there is no server side component. An entry's data and its entire history live together in one InstantDB Storage file (`$files`), not a database field (see docs/data_model.md, Entities); `umkStore` and `entries` remain ordinary rows.
...
- CSP: the same restrictive `Content-Security-Policy` is declared **twice** and must be kept in sync by hand — `public/_headers` (applies once deployed to Cloudflare Pages) and a `<meta http-equiv="Content-Security-Policy">` tag in `index.html` (applies to `npm run dev`/`vite preview`, since Vite never reads `_headers`; confirmed live the hard way — updating only `_headers` left the dev-server CSP unchanged). `connect-src` is limited to Firebase, InstantDB's API (`api.instantdb.com`) and Storage (`files.instantdb.com`, confirmed live — `db.storage`-backed `$files.url` downloads serve from here, not `api.instantdb.com`), and R2's fixed domain pattern. R2's origin is fixed enough to hardcode (`*.r2.cloudflarestorage.com`); any other S3 compatible destination's origin is not known at build time, so using a non-R2 `s3_config` entry requires manually adding that endpoint to `connect-src` in **both** places, or the browser blocks the upload (in whichever of dev/deployed you're testing).
...
src/
  ...
  lib/             smaller supporting modules (blob, validation, limits, totp, commitHash, storage, backup, s3, entryUtils), all .ts
