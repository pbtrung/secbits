...
- **Commit hash** (`commit-hash.test.ts`): canonicalization is key-order independent, one known-answer hash value (48 hex chars), tamper detection.
- **Entry file path/cap** (`storage.test.ts`): `entryFilePath`'s namespacing (a different commit hash yields a different path), `capHistoryArray`'s at/under/over-cap behavior.
...
- **`db.ts`'s pure logic slice** (`db-pure.test.ts`): `fieldsChanged`, `stripNestedHistory`, `buildCommitList` (parent/changed wiring across a commit chain, oldest has no parent, nested history stripped, no redundant `id` on the result), `getVaultStats`, and `buildExportData`'s per-entry history self-filtering (the current version is excluded from its own exported history), its flattening of each remaining commit (no nested `snapshot`, no duplicated `id`, its hash renamed to `commitHash` to match the entry's own top-level field name, see docs/crypto.md, Entry Data File), the exported entry itself dropping `id`, and versioned export shape. These three internals are exported from `db.ts` specifically so they're reachable from a test, since they have no dependency on InstantDB at all.
...
Every exported function that touches InstantDB (`ensureKeyStore`, `fetchUserEntries`, `createUserEntry`, `updateUserEntry`, `deleteUserEntry`, `restoreDeletedUserEntry`, `restoreEntryVersion`, `rotateRootMasterKey`, `rotateUserMasterKey`, `signIn`, and more) has zero test coverage. All of it is fused to real `db.getAuth()`/`db.transact()`/`db.queryOnce()`/`db.storage.uploadFile()`/`db.auth.signInWithIdToken()` calls; there is no mock of `@instantdb/react` anywhere in the repo, so none of this is testable today without either a live InstantDB app or writing a mock layer, neither of which exists. This now also covers the entry file upload/download/swap helpers (`uploadEntryFile`, `downloadEntryFile`, `commitEntryFileSwap`), which are thin wrappers around those same calls. Typing these calls against the schema (`npm run typecheck`) catches shape mismatches at compile time regardless, which is the main practical mitigation for this gap; see `docs/tech_stack.md`.
...
- **`instant.perms.ts`**: needs a real InstantDB app with at least two authenticated test users. Test plan:
  - User A cannot view, update, or delete User B's `entries`, `umkStore`, or `$files` rows.
  - User A cannot create a `umkStore` row with `owner` set to User B, nor an `entries` row with `umk` pointing at User B's `umkStore` row.
  - User A cannot upload a file at a `path` not prefixed with their own `auth.id`, even if linked to one of their own entries.
  - User A cannot update their own `umkStore` row to reassign `owner` to User B, nor their own `entries` row to reassign `umk` to User B's `umkStore` row; `!('owner' in request.modifiedFields)` / `!('umk' in request.modifiedFields)` reject any update that touches that link at all.
